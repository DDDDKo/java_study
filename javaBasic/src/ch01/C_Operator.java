...
public class C_Operator {
	...
	public static void main(String[] args) {
		...
		// 대입 연산자(할당 연산자): 좌항에 우항을 할당
		// 일반 대입 연산자: =
		// 상수에 초기화 할때를 제외하고 좌항이 변수이어야 함
		// 좌항에 우항을 할당
		number = 10;
		...
		// && : AND연산 - 좌항과 우항이 모두 true이면 true, 하나라도 false면 false
		booleanResult = false && false;
		System.out.println(booleanResult);
		booleanResult = a > b && c > d;
		System.out.println(booleanResult);
		// || : OR연산 - 좌항과 우항 중 하나라도 true이면 true, 모두 false면 false
		booleanResult = true || true;
		System.out.println(booleanResult);
		booleanResult = a < b || c > d;
		System.out.println(booleanResult);
		
		// AND, OR 연산자는 앞의 값이 만족하면 뒤의 값을 계산하지(읽지) 않음
		// 증명
		// a: 15, b: 7
