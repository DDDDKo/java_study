...
public class B_Datatype {
	...
	public static void main(String[] args) {
		...
		// 0~65535 부호부를 사용하지 않기 때문에 0부터 시작
		char char1 = 'a';
		char char2 = 97;
		...
		// 자동 형변환 (묵시적 형변환): 작은 데이터 타입의 변수를 큰 데이터 타입의 변수 옮겨 담을 때 발생
		int number = 10;
		...
		item = numbers[0];
		System.out.println(item);  // 배열 생성시 초기화값 0으로 알아서 초기화 돼있음
		numbers[0] = 99;
		System.out.println(item);
		item = numbers[0];
		System.out.println(item);
		...
		numbers2 = numbers;  // numbers2의 주소값(위치)을 numbers의 주소값(위치)으로 변경
		// numbers2 : {99, 0, 0, 0, 0} x
		// numbers2 => [[I@182decdb  O
		...
		// numbers : {99, 0, 0, 0, -99}
		// numbers2 : {99, 0, 0, 0, 0}
		System.out.println(numbers2[4]); // numbers2[4]값이 -99가 나옴 ?
...
//		System.out.println(number2);
		...
		System.out.println(statement.indexOf("하마"));  // 존재하지 않는 문자열은 -1반환
		System.out.println(statement.lastIndexOf("그림"));
		System.out.println(statement.lastIndexOf("하마")); // 존재하지 않는 문자열은 -1반환
		
		// 6. 부분 문자열
		// substring(시작인덱스), substring(시작인덱스, 종료인덱스): 문자열을 원하는 크기만큼 추출
		System.out.println(statement.substring(40));
		System.out.println(statement.substring(3, 10));
		
		// null: 아직 어떠한 값도 지정되지 않은 상태
		String string2 = null;
		System.out.println(string2.substring(48));  // null값에 인덱스 값 자체가 존재하지 않기 때문에 출력 불가능
		
		// **** 참조변수(new, 배열, String)를 사용시 주소(위치)를 무조건 적으로 가진다
		// **** 배열[5]의 크기는 5 0~4까지
	}
}
