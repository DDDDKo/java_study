...
public class B_Package2 {
	...
	public static void main(String[] args) {
		...
		System.out.println(Math.round(3.1415 * 100) / 100.0); // 소수점 3번째 자리에서 반올림
...
//	 	Random random = new Random(10);    10은 시드값 (시드값을 설정하면 알고리즘을 타고 똑같은 값이 출력됨) => 현제 시간을 넣어주면 난수를 생성할 수 있음
		Random random = new Random(); 
		System.out.println(random.nextInt());
		// 범위 제약
		System.out.println(random.nextInt(45));  // 0~44 사이의 난수
		System.out.println(random.nextInt() % 45); // -44~44 사이의 난수
		
		// Lotto: 1 ~ 45, 중복 허용 없이 6개의 수
		...
		// LocalDate 클래스, LocalTime 클래스, LocalDateTime 클래스 => LocalDateTime 클래스는 new연산자를 사용한 생성자를 사용할 수 없음
		// 현재 날짜 및 시간의 인스턴스 생성    LocalDateTime클래스안의 메서드인 now 메서드를 사용하여 인스턴스 생성
		...
		String dateStirng = simpleDateFormat.format(localToDate);
		System.out.println(dateStirng);
		
		// y: 연도, M: 월, d: 일, a: 오전,오후, H: 시간(24시간형태), h: 시간(12시간형태), m: 분, s: 초, E: 요일
		simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd a HH:mm:ss");
		dateStirng = simpleDateFormat.format(localToDate);
		System.out.println(dateStirng);

		// LocalDateTime -> String
		dateStirng = now.format(DateTimeFormatter.ofPattern("yyyy-MM-dd a HH:mm:ss"));
		System.out.println(dateStirng);
