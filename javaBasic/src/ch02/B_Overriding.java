...
// 규칙: 1. 상위 클래스에서 선언된 메서드와 반환타입, 메서드 이름, 매개변수의 조합이 같아야함
...
class SubClass2 extends SuperClass1{
	...
	// @Override : 컴파일러에게 아래에 오는 메서드가 오버라이드 메서드 라는것을 알려주는것
	// 가독성 향상과 개발자의 실수를 방지할 수 있음
	
	@Override
	void InstanceMethod() {
		// super 키워드: 자손 클래스(하위 클래스)에서 조상 클래스(상위 클래스)의 필드 혹은 메서드에 접근할 때 사용
